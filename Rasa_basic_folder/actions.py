...
class ActionValidateCuisine(Action):
	...
	def run(self, dispatcher, tracker, domain):
		list_cuisine = ["chinese","mexican","american","italian","thai","north indian"]
...
class ActionSearchRestaurants(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		cuisines_dict={'american':1,'chinese':25,'mexican':73,'italian':55,'north indian':50,'thai':95}
		#results_cuisine=zomato.restaurant_search(str(budget), lat, lon, str(cuisines_dict.get(cuisine)),"rating","desc", 20)
...
class ActionSendEmail(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		lat=d1["location_suggestions"][0]["latitude"]
		lon=d1["location_suggestions"][0]["longitude"]
		cuisines_dict={'american':1,'chinese':25,'mexican':73,'italian':55,'north indian':50,'thai':85}
		...
		msg.attach(part2)
#		refresh_token, access_token, expires_in = sendmail.get_authorization(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET)
#		logger.warning("refresh_token:{}".format(refresh_token))
#		logger.warning("access_token:{}".format(access_token))
#		logger.warning("expires_in:{}".format(expires_in))
		sendmail.send_mail(gmail_user, to, "Restaurant Details",msg.as_string())
